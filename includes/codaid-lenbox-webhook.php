...
function codaid_lenbox_receive_callback($request_data)
{
	...
	if (isset($status)) {
		...
		if ($status === "success" && $parameters['response']['accepted'] === true) {
			$order->update_status(apply_filters('woocommerce_lenbox10x_process_payment_order_status', 'processing', $order), __('Paiement validé par Lenbox', 'codaid-lenbox-woo'));
		} else {
			$order->update_status(apply_filters('woocommerce_lenbox10x_process_payment_order_status', 'cancelled', $order), __('Reponse Lenbox : ' . $parameters['response']['message'], 'codaid-lenbox-woo'));
		}
		$data['status'] = 'OK';
